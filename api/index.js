...
function next(){a.pause();currentId=null;fetch("/api/next").then(r=>r.json()).then(d=>{if(d.id){resolveUrlFor(d.id)}else{document.getElementById("status").textContent="✅ 队列播完";a.currentTime=0;a.play().catch(()=>{})}}).catch(()=>{});}
function prev(){a.pause();currentId=null;fetch("/api/prev").then(r=>r.json()).then(d=>{if(d.id){resolveUrlFor(d.id)}else{resolveUrlFor(window._prevId||"")}}).catch(()=>{});}
...
document.getElementById("testBtn").addEventListener("click",function(){this.textContent="搜索中...";this.disabled=true;fetch("/api/mcp",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({jsonrpc:"2.0",id:99,method:"tools/call",params:{name:"play",arguments:{keyword:"Time Machine"}}})}).then(r=>r.json()).then(j=>{this.textContent=j.result?.content?.[0]?.text||"未知结果";setTimeout(poll,800)}).catch(()=>{this.textContent="❌ 失败·重试";this.disabled=false})});
