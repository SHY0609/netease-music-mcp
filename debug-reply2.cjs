...
(async()=>{
  ...
  const r = await cmd('Runtime.evaluate', {expression:
    "(function(){var all=document.querySelectorAll('*');var f=[];for(var i=0;i<all.length;i++){var t=(all[i].textContent||'').trim();var br=all[i].getBoundingClientRect();if(all[i].offsetParent&&t==='回复'&&all[i].tagName==='SPAN'){var c=all[i].closest('[class*=comment-item]');var p=c;for(var d=0;d<5;d++){p=p&&p.parentElement;}var pt=(p&&p.textContent||'').slice(0,400);f.push({y:Math.round(br.y),x:Math.round(br.x),hasQQQ:pt.includes('@？？？'),has互:pt.includes('互相关注'),pt:pt.slice(0,60).replace(/\\n/g,' ')});}}return JSON.stringify(f);})()",
    returnByValue:true});
