...
async function ensurePingxiang() {
  ...
  await sleep(2500);
  // 选黎先菜店（精准定位最小子元素）
  await evalText(pxSession, `(function(){var all=document.querySelectorAll("*");var best=null,min=999;for(var i=0;i<all.length;i++){var t=all[i].textContent||"";if(all[i].offsetParent&&t.indexOf("黎先菜店")>=0&&all[i].children.length<min){best=all[i];min=all[i].children.length;}}if(best){best.click();return"ok";}return"nf";})()`);
  await sleep(2500);
  pxReady = true;
