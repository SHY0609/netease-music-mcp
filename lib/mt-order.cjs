 ...
 * @param {string} [opts.addressName] - 收货地址（默认从 MT_DEFAULT_ADDRESS 环境变量读取）
...
async function mtOrder(opts = {}) {
  const addr = process.env.MT_DEFAULT_ADDRESS || "默认地址";
  const { shopName, productName, specs, addressName = addr, remark, useCoupons = true } = opts;
