...
export async function searchSongs(keyword, cookie, limit = 8) {
  ...
  return (data.result?.songs || []).map(s => ({ id: String(s.id), name: s.name, artist: (s.artists || s.ar || []).map(a => a.name).join(" / ") }));
}
...
export async function getPlaylistDetail(id, cookie, limit = 100) {
  ...
  const tracks = (data.playlist?.tracks || []).map(s => ({ id: String(s.id), name: s.name, artist: (s.ar || []).map(a => a.name).join(" / ") }));
...
export async function addToPlaylist(playlistId, trackIds, cookie) {
  ...
  if (code !== 200) throw new Error(`addToPlaylist: code=${code}`);
  return true;
}
...
export async function playCommandReport(roomId, songId, formerSongId, progress, playStatus, clientSeq, cookie) {
  const data = await eapiPost("/api/listen/together/play/command/report", {
    roomId: String(roomId),
    commandInfo: JSON.stringify({
      commandType: "GOTO",
      targetSongId: String(songId),
      formerSongId: String(formerSongId || "0"),
      progress: String(progress || "0"),
      playStatus: playStatus || "playing",
...
/** Add songs to listen-together playlist — 获取当前列表，追加后全量提交（含版本号） */
export async function addSongToList(roomId, songIds, cookie) {
  // 1. 获取当前播放列表
  const status = await listenTogetherStatus(roomId, cookie);
  const pl = status?.data?.playlist || {};
  const currentList = pl.displayList?.result || [];
  const verInfo = pl.version?.[0] || {};
  const userId = verInfo.userId || "";
  const newVer = (verInfo.version || 0) + 1;

  // 2. 追加新歌（去重）
  const newSongs = songIds.map(String).filter(id => !currentList.includes(id));
  if (!newSongs.length) return { code: 200, data: { result: true, message: "already in playlist" } };
  const displayList = [...currentList, ...newSongs];
  ...
  const data = await eapiPost("/api/listen/together/sync/list/command/report", {
    ...
    playlistParam: JSON.stringify({
      commandType: "ADD",
      version: [{ userId, version: newVer }],
      anchorSongId: currentList[0] || "",
      anchorPosition: 0,
      randomList: [],
      displayList,
    }),
